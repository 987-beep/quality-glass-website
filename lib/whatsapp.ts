...
export function orderStatusMessage(o: {
  ...
  switch (o.status) {
    ...
    case "payment_verifying":
      return `Namaste! 🙏 We received your payment screenshot for order ${o.order_no} (${amt}). We're verifying it now and will confirm shortly. ${SIGN}`;
    case "paid":
      return `Namaste! ✅ Payment confirmed for order ${o.order_no} (${amt}). Work has started on your order — we'll update you at every step. ${SIGN}`;
    case "in_production":
      return `Your order ${o.order_no} is in production right now 🔨 — it's coming out beautifully. We'll message you the moment it's ready. ${SIGN}`;
    case "ready_for_pickup":
      return `Great news! 🎉 Your order ${o.order_no} is READY for pickup at Quality Glass Emporium & Photo Framing Center, ${ADDR}. ${SHOP.hours}. Please show this message at the counter. ${SIGN}`;
    case "out_for_delivery":
      return `Your order ${o.order_no} (${amt}) is out for delivery 🛵 — it will reach you shortly. Please keep your phone nearby. ${SIGN}`;
    case "completed":
    ...
    case "cancelled":
      return `Your order ${o.order_no} has been cancelled. If this was a mistake, just message us or place a fresh order anytime — we're happy to help. ${SIGN}`;
    default:
      return `Namaste! 🙏 An update on your order ${o.order_no} (${amt}): it is now "${o.status}". ${SIGN}`;
  }
}
