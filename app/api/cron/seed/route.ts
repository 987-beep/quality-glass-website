...
// This runs as a Vercel cron job — no user interaction needed
const BASE = "https://37gsnq6s.ap-southeast.insforge.app";
...
export async function GET(request: Request) {
  ...
  // Allow secret-based trigger OR Vercel cron
  const url = new URL(request.url);
  const secret = url.searchParams.get("secret");
  const cronHeader = request.headers.get("x-vercel-cron");
  if (secret !== "qg-seed-2026" && !cronHeader) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  try {
    // Step 1: Delete old data
    logs.push("🗑️ Clearing old data...");
    for (const table of ["product_images", "frame_options", "products", "categories"]) {
      const { data } = await api(`/rest/v1/${table}?select=id`);
      const rows = data as { id: string }[] | null;
      if (Array.isArray(rows)) {
        for (const row of rows) {
          await api(`/rest/v1/${table}?id=eq.${row.id}`, { method: "DELETE" });
          await delay(800);
        }
        logs.push(`  Deleted ${rows.length} from ${table}`);
      }
      await delay(1000);
    }

    // Step 2: Create categories
    logs.push("📁 Creating categories...");
    const catMap: Record<string, string> = {};
    for (const cat of CATEGORIES) {
      await delay(1500);
      const { data, ok, raw } = await api("/rest/v1/categories", {
        method: "POST",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ slug: cat.slug, name: cat.name, is_active: true }),
      });
      if (!ok) {
        logs.push(`  ❌ ${cat.slug}: ${raw.substring(0, 200)}`);
        continue;
      }
      const rows = data as { id: string }[] | null;
      const id = Array.isArray(rows) ? rows[0]?.id : (data as { id?: string })?.id;
      if (id) catMap[cat.slug] = id;
      logs.push(`  ✅ ${cat.slug}`);
    }
    logs.push(`✅ ${Object.keys(catMap).length} categories`);

    // Step 3: Create products
    logs.push("📦 Creating 30 products...");
    let total = 0;
    for (const [catSlug, items] of Object.entries(PRODUCTS)) {
      const catId = catMap[catSlug];
      if (!catId) {
        logs.push(`  ❌ No category for ${catSlug}`);
        continue;
      }
      for (const p of items) {
        await delay(1500);
        const { ok, raw } = await api("/rest/v1/products", {
          method: "POST",
          body: JSON.stringify({
            slug: p.s,
        ...
        if (!ok) {
          logs.push(`  ❌ ${p.s}: ${raw.substring(0, 200)}`);
          continue;
        }
        total++;
        logs.push(`  ✅ ${p.s}`);
