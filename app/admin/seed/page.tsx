...
interface P { s: string; en: string; hi: string; d: string; p: number; t: string; f?: boolean }
...
export default function SeedPage() {
  const [log, setLog] = useState<string[]>(["⏳ Checking database..."]);
  const [status, setStatus] = useState<"checking" | "running" | "done" | "ready" | "error">("checking");
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!user?.id) {
          msgs.push("❌ Not logged in. Redirecting to login...");
          setLog([...msgs]);
          window.location.href = "/login";
          return;
        }
        msgs.push(`✅ Logged in as ${user.id.substring(0, 8)}...`);
        setLog([...msgs]);
        ...
        if (prof?.role !== "admin") {
          msgs.push(`❌ Not admin (role: ${prof?.role || "none"}). Only admin can seed.`);
          msgs.push("Sign in with your Google account (ajmalnic@gmail.com) that is marked as admin.");
          setLog([...msgs]);
          setStatus("error");
          setError("Not an admin account. Please sign in with your admin Google account.");
          return;
        }
        ...
        const { data: existingCats } = await client.database.from("categories").select("id");
        if (Array.isArray(existingCats) && existingCats.length >= 5) {
          msgs.push(`✅ Database already has ${existingCats.length} categories. Ready!`);
          setLog([...msgs]);
          setStatus("ready");
        ...
        // 3. Seed!
        setStatus("running");
        msgs.push("🌱 Database is empty — seeding now...");
        setLog([...msgs]);
        ...
        for (const table of ["product_images", "frame_options", "products", "categories"]) {
          ...
          msgs.push(`   Cleared ${(data as unknown[])?.length || 0} rows`);
          setLog([...msgs]);
          await delay(800);
        }

        // Create categories
        msgs.push("📁 Creating 10 categories...");
        setLog([...msgs]);
        ...
        for (const cat of CATEGORIES) {
          ...
          if (error) { msgs.push(`   ❌ ${cat.slug}: ${error.message || JSON.stringify(error)}`); setLog([...msgs]); continue; }
        ...
        // Create products
        msgs.push("📦 Creating 30 products (this takes ~60s)...");
        setLog([...msgs]);
        let total = 0;
        for (const [catSlug, items] of Object.entries(PRODUCTS)) {
          ...
          for (const p of items) {
            ...
            if (error) { msgs.push(`   ❌ ${p.s}: ${error.message || JSON.stringify(error)}`); setLog([...msgs]); continue; }
            total++;
            msgs.push(`   ✅ ${total}/30 ${p.s}`);
            setLog([...msgs]);
          }
        }

        msgs.push(`\n🎉 DONE! ${Object.keys(catMap).length} categories, ${total} products created!`);
        msgs.push("Redirecting to admin panel...");
        setLog([...msgs]);
        setStatus("done");
  ...
  return (
    <main className="flex min-h-[100svh] flex-col items-center justify-center px-5 py-24">
      <div className="w-full max-w-2xl rounded-2xl border border-gold/40 bg-ink-2 p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="font-serif text-2xl text-ivory">
            ...
            {status === "running" && "🌱 Seeding Database..."}
        ...
        <pre className="max-h-[60vh] overflow-auto rounded-lg border border-ivory/10 bg-ink p-4 text-xs leading-5 text-ivory/70 font-mono">
          {log.join("\n")}
        </pre>
        ...
        {error && (
          <div className="mt-4 rounded-lg border border-red-500/40 bg-red-500/10 p-4">
            ...
            <div className="mt-3 flex gap-3">
              <a href="/login" className="rounded-full bg-gold px-4 py-2 text-xs font-bold text-ink">
                Sign in as Admin
              </a>
              <button onClick={() => window.location.reload()} className="rounded-full border border-ivory/20 px-4 py-2 text-xs text-ivory/60 hover:text-ivory">
                Retry
              </button>
            </div>
          </div>
        ...
        {status === "done" && (
          <div className="mt-6 text-center">
            <p className="text-lg text-gold font-serif">✅ All products created successfully!</p>
            <a href="/" className="mt-4 inline-block rounded-full bg-gold px-8 py-3 text-sm font-bold uppercase text-ink hover:bg-gold-light transition-colors">
              View Website →
            </a>
          </div>
        )}
